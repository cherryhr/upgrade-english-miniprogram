...
def main():
    ...
    if test_audio and len(test_audio) > 1000:
        ...
        test_file = os.path.join(LOCAL_AUDIO_DIR, "test_tencent.mp3")
    ...
    for i, item in enumerate(texts):
        ...
        if audio and len(audio) > 1000:
            ...
            # 上传到COS
            try:
                cos_client.put_object(Bucket=COS_BUCKET, Body=open(local_path, 'rb'), Key=cos_key, ContentType='audio/mpeg')
                print(f"  ✅ ({len(audio)//1024}KB) ☁️ COS")
            except Exception as e:
                print(f"  ✅ ({len(audio)//1024}KB) ⚠️ COS失败")
            success += 1
        else:
            print(f"  ❌ 失败")
            failed += 1
